...
class CenterService
{
    ...
    {
        $centerId = DB::table('admin_centers')
    ...
    public function show()
    {
        ...
        // رابط الصورة الصحيح (أو null إن ما في صورة)
        $imageUrl = $center->image
            ? Storage::disk('public')->url(ltrim($center->image, '/'))
            : null;

        // متوسط التقييم (0..5) من جدول ratings
        ...
        $payload['rating_average']  = $avg ? round((float) $avg, 1) : 0.0;

        return $this->unifiedResponse(true, 'Center fetched.', $payload);
    }
    ...
    public function update(array $data)
    {
        ...
        // نرجع نفس تنسيق show لضمان اتساق الـ API
        $imageUrl = $center->image
            ? Storage::disk('public')->url(ltrim($center->image, '/'))
            : null;
        ...
        return $this->unifiedResponse(true, 'Center updated.', $payload);
    }
    ...
    public function uploadImage($file)
    {
        ...
        if (!$file || !$file->isValid()) {
            return $this->unifiedResponse(false, 'No valid file uploaded.', [], [], 422);
        }

        $allowed = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
        if (!in_array($file->getMimeType(), $allowed)) {
            return $this->unifiedResponse(false, 'Invalid file type. Only images are allowed.', [], [], 422);
        }
        ...
        // تخزين الصورة على public/centers
        ...
        $payload = $center->toArray();
        $payload['image_url'] = Storage::disk('public')->url(ltrim($center->image, '/'));
    ...
    public function workingHoursIndex()
    {
        ...
        $hours = $center->workingHours()
            ->orderByRaw("FIELD(day_of_week,'Saturday','Sunday','Monday','Tuesday','Wednesday','Thursday','Friday')")
            ->get();

        return $this->unifiedResponse(true, 'Center working hours fetched.', $hours);
    }
